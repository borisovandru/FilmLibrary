...
class MovieInfoViewModel(private val liveDataToObserver: MutableLiveData<AppState> = MutableLiveData()) :
    ...
    private val callBack = object :
        ...
        override fun onResponse(call: Call<FactDTO>, response: Response<FactDTO>) {
            Log.v("Debug1", "MovieInfoViewModel onResponse")
            val serverResponse: FactDTO? = response.body()
            liveDataToObserver.postValue(
        ...
        private fun checkResponse(serverResponse: FactDTO): AppState {
            ...
            return if (fact == -1) {
                ...
                if (serverResponse.dateRelease != "") {
                    val localDate = LocalDate.parse(serverResponse.dateRelease,
    ...
    /*fun convertDtoToModel(movieDTO: MovieDTO): Movie {
        Log.v("Debug1", "MovieInfoViewModel convertDtoToModel")
        val fact: FactDTO = movieDTO.fact!!
        return Movie(fact.id.toInt(),
            fact.title
        )
    }*/
    ...
    fun getMovieFromRemoteSource() {
        Log.v("Debug1", "MovieInfoViewModel getMovieFromRemoteSource")
        liveDataToObserver.value = AppState.Loading
        repository.getMovieFromRemoteServer2(movieId, LANG_VALUE, callBack)
        /*Thread {
            liveDataToObserver.postValue(
                (repository.getMovieFromRemoteServer(movieId))
            )
